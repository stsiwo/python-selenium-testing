...
@pytest.fixture(scope="module")
def login_for_update_blog(login):
    print("*** login_for_blog_management setup ***")
    home_page = login
    home_page.driver.get(cfg.blog_management_url)

    blog_management_page = BlogManagementPage(home_page.driver, independent=False)

    blog_management_page.click_edit_icon_of_one_of_the_blog_items()
