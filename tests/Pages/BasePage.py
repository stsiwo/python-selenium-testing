...
class BasePage(object):
    ...
    def wait_for_element_to_have_text(self, locator: str = None, text: str = ''):
        if locator is None:
            print("wait for no element. you may want to specify locator as argument to wait")
        if locator not in self.element_locators:
            raise Exception('locator you provide is not available. available locators: %s' % self.element_locators)
        if locator is not None:
            WebDriverWait(self.driver, 500).until(
                    EC.text_to_be_present_in_element(self.element_locators[locator], text)
                    )

    def wait_for_element_to_be_visible(self, locator: str = None, element=None):
        if locator is None:
            print("wait for no element. you may want to specify locator as argument to wait")
        if element is None:
            print("wait for no element. you may want to specify locator as argument to wait")
        if locator is not None and locator not in self.element_locators:
            raise Exception('locator you provide is not available. available locators: %s' % self.element_locators)
        if locator is not None:
            WebDriverWait(self.driver, 500).until(
                    EC.visibility_of(self.element_locators[locator])
                    )
    ...
    def wait_for_element_to_be_clickable(self, locator: str = None):
        if locator is None:
            print("wait for no element. you may want to specify locator as argument to wait")
        if locator not in self.element_locators:
            raise Exception('locator you provide is not available. available locators: %s' % self.element_locators)
        if locator is not None:
            WebDriverWait(self.driver, 500).until(
                    EC.element_to_be_clickable(self.element_locators[locator])
                    )
